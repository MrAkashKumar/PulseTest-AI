...
function Celebration({ percentage }) {
  ...
  return <div className="micro-celebration" aria-live="polite"><div className="confetti-field">{Array.from({ length: 18 }, (_, index) => <i key={index} style={{ "--i": index }} />)}</div><span>✦</span><b>{message}</b></div>;
}
...
export default function ResultsReport({ exam, exams, research, onExit, onLibrary, onProgress, onNeedKey, onTutor }) {
  ...
  return <section className="results-page report-v2">{celebrate && <Celebration percentage={result.percentage} />}
