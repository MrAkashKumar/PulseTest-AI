...
export function buildQuestionPrompt({ count, startNumber, subjects, difficulty, mode, customPrompt, researchBrief, coveredTopics, adaptiveProfile, seenQuestionSignatures, liveVerificationMode }) {
  return `${PANEL}
...
- integratedSubjects must be short official subject labels only. sourceTags must be concise NEET-PG topic labels only, usually 1–4 words each. Never include URLs, citations, brackets, year-note sentences or long evidence fragments inside sourceTags.
- Explanation must teach the decisive reasoning. whyOthersWrong must contain exactly the three incorrect options only.
- trap must explicitly say why a strong candidate could be lured into the wrong path and what subtle cue defeats that trap; clue points to the decisive stem detail; memoryTip is short and memorable without being silly or medically inaccurate.
...
PAST QUESTION SIGNATURES — do not generate close variants of these:
${seenQuestionSignatures?.length ? seenQuestionSignatures.slice(-80).join("\n") : "No prior signatures"}
...
VERIFICATION MODE:
${liveVerificationMode || "No live search is available in this request. Prefer stable established standards, avoid fragile guideline-sensitive cut-offs unless already grounded in the research brief, and discard any candidate whose answer depends on uncertain or recently changed numbers."}

Before returning, silently audit the complete batch for answer-position balance, duplicate concepts, near-duplicate stems, internal contradictions, bias and answer-key certainty. Replace failures. Return only the required structured data. The UI will keep the answer key hidden until final submission.`;
}
